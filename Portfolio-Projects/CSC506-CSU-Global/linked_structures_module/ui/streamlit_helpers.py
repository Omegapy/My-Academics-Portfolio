...
from linked_structures_module.data.dataset_manager import preview_dataset
from linked_structures_module.models.operation_result import OperationResult
...
def render_analysis_markdown_file(
    ...
    chart_paths = {
        "COMMON_OPERATION_CHART": charts_dir / "common_operation_runtime.png",
        "STRUCTURE_SPECIFIC_CHART": charts_dir / "structure_specific_runtime.png",
        "OPERATION_WINNER_HEATMAP": charts_dir / "operation_winner_heatmap.png",
        "STRUCTURE_PROFILE_TABLE": charts_dir / "structure_profile_comparison.png",
    }
    # Build benchmark results table
    if benchmark_df is not None and not benchmark_df.empty:
        # Local import to avoid circular dependency at package import time.
        from linked_structures_module.analysis.report_generator import build_benchmark_table
        placeholders["BENCHMARK_RESULTS_TABLE"] = build_benchmark_table(
            benchmark_df
        )
    # Build operation winners table
    if winners_df is not None and not winners_df.empty:
        from linked_structures_module.analysis.report_generator import build_operation_winners_table
