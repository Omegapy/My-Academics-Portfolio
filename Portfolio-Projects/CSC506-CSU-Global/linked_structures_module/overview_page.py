...
# Import-safe Streamlit page renderer for the Linked Structures module.
# The page exposes seven local tabs inside the Portfolio Module 8 root app:
#   1. Overview
#   2. Dataset Builder
...
# The root Portfolio Module 8 app is the only runnable Streamlit entry point.
# -------------------------------------------------------------------------
...
"""Linked Structures Streamlit page for Portfolio Module 8."""
...
from linked_structures_module.analysis.benchmark_structures import (  # noqa: E402
    DEFAULT_REPEATS,
    DEFAULT_SIZES,
...
from linked_structures_module.data.dataset_manager import (  # noqa: E402
    DATASET_TYPES,
...
from linked_structures_module.data_structures import (  # noqa: E402
    Deque,
    LinkedList,
    Queue,
    Stack,
)
from linked_structures_module.models.operation_result import OperationResult  # noqa: E402
from linked_structures_module.ui.streamlit_helpers import (  # noqa: E402
...
def _init_session_state() -> None:
    ...
    defaults: dict[str, object] = {
        "ls_dataset": None,
        "ls_dataset_type": "sequential",
        "ls_dataset_size": DEFAULT_DATASET_SIZE,
        "ls_structure_state": {},
        "ls_structure_objects": {},
        "ls_active_structure": "Stack",
        "ls_last_operation_result": None,
        ...
        "ls_last_section": None,
        "ls_benchmark_df": None,
        "ls_operation_winners_df": None,
    }
...
def _record_and_locate(section_id: str, *args, **kwargs) -> None:
    ...
    """
    st.session_state["ls_last_operation_result"] = _record_op(*args, **kwargs)
    st.session_state["ls_last_section"] = section_id
...
def _maybe_render_section_result(section_id: str, structure_name: str) -> None:
    ...
    """
    if st.session_state.get("ls_last_section") != section_id:
        return
    result = st.session_state.get("ls_last_operation_result")
...
def render_linked_structures_page() -> None:
    """Render the integrated Linked Structures page.
    ...
    * **LinkedList** — a custom doubly linked list with front and rear inserts, search,
    ...
    with tabs[1]:
        ...
        with col_left:
            ...
            dtype = st.selectbox(
                "Dataset type",
                DATASET_TYPES,
                index=DATASET_TYPES.index(st.session_state["ls_dataset_type"])
                if st.session_state["ls_dataset_type"] in DATASET_TYPES
                else 0,
                format_func=lambda dt: _DATASET_TYPE_LABELS.get(dt, dt),
                key="ls_dataset_type_widget",
            )
            #--- dataset size 
            size_value = st.number_input(
                ...
                min_value=1,
                max_value=1_000_000,
                value=int(st.session_state["ls_dataset_size"]),
                step=100,
                key="ls_dataset_size_widget",
            )
            #--- quick presets 
            st.caption("Quick presets")
            preset_cols = st.columns(4)
            for idx, preset in enumerate([1_000, 5_000, 10_000, 50_000]):
                if preset_cols[idx].button(f"{preset:,}", key=f"ls_preset_{preset}"):
                    size_value = preset
                    st.session_state["ls_dataset_size"] = preset
            ...
            if dtype == "manual":
                manual_text = st.text_area(
                    ...
                    height=120,
                    key="ls_manual_dataset_text",
                )
            #--- generate dataset button 
            if st.button("Generate Dataset", type="primary", key="ls_generate_dataset"):
                try:
                    if dtype == "manual":
                    ...
                    st.session_state["ls_dataset"] = data
                    st.session_state["ls_dataset_type"] = dtype
                    st.session_state["ls_dataset_size"] = len(data)
                    st.success(f"Generated {len(data):,} values.")
                except (ValueError, TypeError) as exc:
                    st.error(f"Could not generate dataset: {exc}")
        #--- dataset info 
        with col_right:
            if st.session_state["ls_dataset"] is None:
                st.info("Generate or paste a dataset on the left to get started.")
            else:
                render_dataset_info(
                    data=st.session_state["ls_dataset"],
                    dataset_type=_DATASET_TYPE_LABELS.get(
                        st.session_state["ls_dataset_type"],
                        st.session_state["ls_dataset_type"],
                    ),
                    size=st.session_state["ls_dataset_size"],
                )
            ...
            with controls_col:
                structure_name = st.selectbox(
                    ...
                    index=_STRUCTURE_NAMES.index(
                        st.session_state["ls_active_structure"]
                    ),
                    key="ls_active_structure",
                )
                ...
                if st.button(
                    ...
                    fresh = _build_fresh_structure(
                        structure_name, list(st.session_state["ls_dataset"])
                    )
                    st.session_state["ls_structure_objects"][structure_name] = fresh
                    st.session_state["ls_last_operation_result"] = None
                    st.session_state["ls_last_section"] = None
                    st.success(
                        f"Loaded {len(st.session_state['ls_dataset']):,} values "
                        f"into a fresh {structure_name}."
                    )
                #--- structure object dataset
                structure = st.session_state["ls_structure_objects"].get(structure_name)
                if structure is None:
                    ...
                    if structure_name == "Stack":
                        ...
                        if push_col[0].button("push", key="ls_op_push"):
                            _record_and_locate(
                                "stack_with_value",
                                structure_name,
                                "push",
                        ...
                        #--- pop operation 
                        if no_cols[0].button("pop", key="ls_op_pop"):
                            _record_and_locate(
                                "stack_no_input",
                                structure_name,
                                "pop",
                        ...
                        #--- peek operation 
                        if no_cols[1].button("peek", key="ls_op_peek"):
                            _record_and_locate(
                                "stack_no_input",
                                structure_name,
                                "peek",
                        ...
                        #--- is_empty operation 
                        if no_cols[2].button("isEmpty", key="ls_op_is_empty"):
                            _record_and_locate(
                                "stack_no_input",
                                structure_name,
                                "isEmpty",
                        ...
                        #--- clear operation 
                        if no_cols[3].button("clear", key="ls_op_clear"):
                            _record_and_locate(
                                "stack_no_input",
                                structure_name,
                                "clear",
                    ...
                    #--- queue operations 
                        ...
                        if enq_col[0].button("enqueue", key="ls_q_enq"):
                            _record_and_locate(
                                "queue_with_value",
                                structure_name,
                                "enqueue",
                        ...
                        no_cols = st.columns(4)
                        #--- dequeue operation 
                        if no_cols[0].button("dequeue", key="ls_q_deq"):
                            _record_and_locate(
                                "queue_no_input",
                                structure_name,
                                "dequeue",
                        ...
                        #--- front operation 
                        if no_cols[1].button("front", key="ls_q_front"):
                            _record_and_locate(
                                "queue_no_input",
                                structure_name,
                                "front",
                        ...
                        #--- is_empty operation 
                        if no_cols[2].button("isEmpty", key="ls_q_isempty"):
                            _record_and_locate(
                                "queue_no_input",
                                structure_name,
                                "isEmpty",
                        ...
                        #--- clear operation 
                        if no_cols[3].button("clear", key="ls_q_clear"):
                            _record_and_locate(
                                "queue_no_input",
                                structure_name,
                                "clear",
                    ...
                    #--- deque operations 
                        ...
                        if add_cols[0].button("addFront", key="ls_d_addF"):
                            _record_and_locate(
                                "deque_with_value",
                                structure_name,
                                "addFront",
                        ...
                        #--- add rear operation 
                        if add_cols[1].button("addRear", key="ls_d_addR"):
                            _record_and_locate(
                                "deque_with_value",
                                structure_name,
                                "addRear",
                        ...
                        if rem_cols[0].button("removeFront", key="ls_d_remF"):
                            _record_and_locate(
                                "deque_no_input",
                                structure_name,
                                "removeFront",
                        ...
                        if rem_cols[1].button("removeRear", key="ls_d_remR"):
                            _record_and_locate(
                                "deque_no_input",
                                structure_name,
                                "removeRear",
                        ...
                        if peek_cols[0].button("peekFront", key="ls_d_pF"):
                            _record_and_locate(
                                "deque_no_input",
                                structure_name,
                                "peekFront",
                        ...
                        #--- peek rear operation     
                        if peek_cols[1].button("peekRear", key="ls_d_pR"):
                            _record_and_locate(
                                "deque_no_input",
                                structure_name,
                                "peekRear",
                        ...
                        if meta_cols[0].button("isEmpty", key="ls_d_isempty"):
                            _record_and_locate(
                                "deque_no_input",
                                structure_name,
                                "isEmpty",
                        ...
                        #--- clear operation     
                        if meta_cols[1].button("clear", key="ls_d_clear"):
                            _record_and_locate(
                                "deque_no_input",
                                structure_name,
                                "clear",
                    ...
                    #--- linked list operations 
                        ...
                        if ins_col[0].button("insert", key="ls_ll_ins"):
                            complexity = (
                        ...
                        if del_mode in ("before", "after"):
                            del_anchor = int(
                                st.number_input(
                                    ...
                                    max_value=1_000_000,
                                    value=0,
                                    key="ls_ll_delete_anchor",
                                )
                            )
                        #--- delete operation    
                        del_col = st.columns([1, 3])
                        if del_col[0].button("delete", key="ls_ll_del"):
                            del_complexity = (
                        ...
                        search_val = int(
                            st.number_input(
                                ...
                                max_value=1_000_000,
                                value=0,
                                key="ls_ll_search_value",
                            )
                        )
                        #--- search operation 
                        search_col = st.columns([1, 3])
                        if search_col[0].button("search", key="ls_ll_srch"):
                            _record_and_locate(
                                "ll_search",
                                structure_name,
                                "search",
                        ...
                        reverse_display = st.checkbox(
                            "reverse", value=False, key="ls_ll_rev"
                        )
                        disp_col = st.columns([1, 3])
                        if disp_col[0].button("display", key="ls_ll_disp"):
                            _record_and_locate(
                                "ll_display",
                                structure_name,
                        ...
                        if meta_cols[0].button("isEmpty", key="ls_ll_isempty"):
                            _record_and_locate(
                                "ll_no_input",
                                structure_name,
                                "isEmpty",
                        ...
                        #--- clear operation 
                        if meta_cols[1].button("clear", key="ls_ll_clear"):
                            _record_and_locate(
                                "ll_no_input",
                                structure_name,
                                "clear",
                        ...
                        #--- length operation 
                        if meta_cols[2].button("len", key="ls_ll_len"):
                            _record_and_locate(
                                "ll_no_input",
                                structure_name,
                                "__len__",
    ...
    with tabs[4]:
        ...
        with config_col:
            ...
            chosen_sizes = st.multiselect(
                ...
                key="ls_benchmark_sizes",
            )
            preset_cols = st.columns(2)
            if preset_cols[0].button("Preview [100, 500]", key="ls_preview_sizes"):
                chosen_sizes = [100, 500]
            if preset_cols[1].button(
                "Full required [1k, 5k, 10k, 50k]",
                key="ls_full_sizes",
            ):
            ...
            repeats = st.number_input(
                ...
                key="ls_benchmark_repeats",
            )
            include_specific = st.checkbox(
                "Include structure-specific workloads",
                value=True,
                key="ls_include_specific_workloads",
            )
        ...
        with action_col:
            ...
            run_clicked = st.button(
                "Run Benchmark",
                type="primary",
                key="ls_run_benchmark",
            )
            save_clicked = st.button("Save Results to CSV", key="ls_save_benchmark")
            load_clicked = st.button("Load saved CSV", key="ls_load_benchmark")
        ...
        if run_clicked:
            if not chosen_sizes:
                ...
                progress.empty()
                st.session_state["ls_benchmark_df"] = df
                st.session_state["ls_operation_winners_df"] = compute_operation_winners(df)
                st.success(f"Benchmark complete: {len(df)} rows.")
        #--- save benchmark operation   
        if save_clicked:
            if st.session_state.get("ls_benchmark_df") is None:
                st.error("No benchmark in memory. Run the benchmark first.")
            else:
                save_results_csv(st.session_state["ls_benchmark_df"], _BENCHMARK_CSV)
                save_operation_winners_csv(
                    st.session_state["ls_operation_winners_df"], _WINNERS_CSV
                )
                st.success(
        ...
        if load_clicked:
            if not _BENCHMARK_CSV.exists():
                ...
                if _WINNERS_CSV.exists():
                    st.session_state["ls_operation_winners_df"] = pd.read_csv(_WINNERS_CSV)
                else:
                    st.session_state["ls_operation_winners_df"] = compute_operation_winners(df)
                st.success(f"Loaded {len(df)} rows from `{_BENCHMARK_CSV.name}`.")
        #--- benchmark results display 
        df = st.session_state.get("ls_benchmark_df")
        if df is not None and not df.empty:
            ...
            st.divider()
            render_benchmark_charts(
                df, winners_df=st.session_state.get("ls_operation_winners_df")
            )
    ...
    with tabs[5]:
        ...
        df = st.session_state.get("ls_benchmark_df")
        winners_df = st.session_state.get("ls_operation_winners_df")
    ...
    # End of File
    #
# --------------------------------------------------------------- end render_linked_structures_page()
