# File: overview_page.py | Author: Alexander Ricciardi | Date: 2026-05-05
#
...
#
# --- Usage / Integration ---
# Rendered by: Portfolio-Module-8/streamlit_app.py
# -------------------------------------------------------------------------
#
# --- Apache-2.0 ---
...
"""Import-safe Graph Algorithms page module for Portfolio Module 8.

This module adapts the CTA-7 Graph Representation and Algorithm Tool so it
can render inside the single Portfolio Module 8 Streamlit application.
"""
...
# SETUP: resolve artifacts relative to this Module 8 package.
...
# DATASET MANAGER
# ==============================================================================
from graph_algorithms_module.data.graph_dataset_manager import (
    build_graph,
...
from graph_algorithms_module.models.shortest_path_result import ShortestPathResult
from graph_algorithms_module.models.traversal_result import TraversalResult

# ==============================================================================
# STREAMLIT HELPERS
# ==============================================================================
from graph_algorithms_module.ui.streamlit_helpers import (
    build_graphviz_dot,
...
_DEFAULTS: dict[str, object] = {
    "ga_vertices": None,
    "ga_edges": None,
    "ga_dataset_name": "None",
    "ga_directed": False,
    "ga_list_graph": None,
    "ga_matrix_graph": None,
    "ga_list_history": [],
    "ga_matrix_history": [],
    "ga_last_list_operation_result": None,
    "ga_last_matrix_operation_result": None,
    "ga_structure_vertices": None,
    "ga_structure_edges": None,
    "ga_structure_list_graph": None,
    "ga_structure_matrix_graph": None,
    "ga_structure_list_history": [],
    "ga_structure_matrix_history": [],
    "ga_structure_history": [],
    "ga_last_structure_list_operation_result": None,
    "ga_last_structure_matrix_operation_result": None,
    "ga_last_structure_operation_results": None,
    "ga_last_structure_section": None,
    "ga_structure_sync_warning": "",
    "ga_traversal_vertices": None,
    "ga_traversal_edges": None,
    "ga_traversal_list_graph": None,
    "ga_traversal_matrix_graph": None,
    "ga_traversal_result_graph": None,
    "ga_traversal_result": None,
    "ga_traversal_list_result": None,
    "ga_traversal_matrix_result": None,
    "ga_shortest_vertices": None,
    "ga_shortest_edges": None,
    "ga_shortest_list_graph": None,
    "ga_shortest_matrix_graph": None,
    "ga_shortest_graph_config": None,
    "ga_shortest_path_result_graph": None,
    "ga_shortest_path_result": None,
    "ga_shortest_comparison_df": None,
    "ga_traversal_demo_result": None,
    "ga_shortest_demo_result": None,
    "ga_shortest_visual_run_id": 0,
    "ga_list_demo_result": None,
    "ga_matrix_demo_result": None,
    "ga_benchmark_df": None,
    "ga_operation_winners_df": None,
    "ga_scaling_df": None,
    "ga_benchmark_validation": None,
    "ga_chart_paths": [],
    "ga_benchmark_sizes": [25, 50, 100],
    "ga_benchmark_kinds": list(DEFAULT_GRAPH_KINDS),
    "ga_benchmark_repeats": 1,
    "ga_benchmark_directed": False,
    "ga_benchmark_include_bellman_ford": True,
    "ga_builder_vertex_count": _DEFAULT_RANDOM_VERTEX_COUNT,
    "ga_builder_seed": DEFAULT_SEED,
    "ga_shortest_demo_name": _POSITIVE_DISTANCE_DEMO_NAME,
}

# -------------------------------------------------------------------------------- _init_session_state()
def _init_session_state() -> None:
    """Initialize Graph Algorithms session state defaults.
...
def _graph_data_ready() -> bool:
    ...
    """
    return st.session_state["ga_vertices"] is not None and st.session_state["ga_edges"] is not None
# --------------------------------------------------------------------------------
...
def _active_graph(representation: str) -> WeightedGraph | None:
    ...
    """
    key = "ga_list_graph" if representation == "list" else "ga_matrix_graph"
...
def _load_graphs(vertices: list[str], edges: list[GraphEdge], *, directed: bool) -> None:
    ...
    Returns:
        None.
    """
    st.session_state["ga_vertices"] = vertices
    st.session_state["ga_edges"] = edges
    st.session_state["ga_directed"] = directed
    st.session_state["ga_list_graph"] = build_graph("list", vertices, edges, directed=directed)
    st.session_state["ga_matrix_graph"] = build_graph("matrix", vertices, edges, directed=directed)
# --------------------------------------------------------------------------------
...
def _graph_for_lab(prefix: str, representation: str) -> WeightedGraph | None:
    ...
    Returns:
        Lab graph or None.
    """
    key = f"ga_{prefix}_{representation}_graph"
    graph = st.session_state[key]
    return graph if graph is not None else None
# --------------------------------------------------------------------------------
...
def _store_lab_graphs(
    ...
    *,
    ...
    Returns:
        None.
    """
    st.session_state[f"ga_{prefix}_vertices"] = vertices
    st.session_state[f"ga_{prefix}_edges"] = edges
    st.session_state[f"ga_{prefix}_list_graph"] = build_graph(
        "list",
        vertices,
        edges,
        directed=directed,
    )
    st.session_state[f"ga_{prefix}_matrix_graph"] = build_graph(
        "matrix",
        vertices,
        edges,
        directed=directed,
...
def _ensure_structure_graphs() -> None:
    ...
    """
    if (
        st.session_state["ga_structure_list_graph"] is not None
        and st.session_state["ga_structure_matrix_graph"] is not None
    ):
        return
...
def _ensure_traversal_graphs() -> None:
    ...
    """
    if (
        st.session_state["ga_traversal_list_graph"] is not None
        and st.session_state["ga_traversal_matrix_graph"] is not None
    ):
        return
...
def _ensure_shortest_demo_graphs(demo_name: str) -> None:
    ...
    config = (demo_name,)
    if (
        st.session_state["ga_shortest_graph_config"] == config
        and st.session_state["ga_shortest_list_graph"] is not None
    ):
        return
    ...
    st.session_state["ga_shortest_graph_config"] = config
    st.session_state["ga_shortest_path_result"] = None
    st.session_state["ga_shortest_path_result_graph"] = None
    st.session_state["ga_shortest_demo_result"] = None
    st.session_state["ga_shortest_comparison_df"] = None
# --------------------------------------------------------------------------------
...
def _move_visual_step(state_key: str, slider_key: str, delta: int, step_count: int) -> None:
    ...
    Returns:
        None.
    """
    if not state_key.startswith("ga_"):
        state_key = f"ga_{state_key}"
    if not slider_key.startswith("ga_"):
        slider_key = f"ga_{slider_key}"
...
def _append_history(key: str, message: str) -> None:
    ...
    Returns:
        None.
    """
    state_key = key if key.startswith("ga_") else f"ga_{key}"
    history = list(st.session_state[state_key])
    history.insert(0, message)
    st.session_state[state_key] = history[:8]
# --------------------------------------------------------------------------------
...
def _sync_structure_source_state() -> None:
    ...
    if graph is None:
        st.session_state["ga_structure_vertices"] = None
        st.session_state["ga_structure_edges"] = None
        return
    st.session_state["ga_structure_vertices"] = graph.vertices()
    st.session_state["ga_structure_edges"] = graph.edges()
# --------------------------------------------------------------------------------
...
def _show_edge_weights_for_active_dataset() -> bool:
    ...
    """
    return st.session_state["ga_dataset_name"] not in {
        _TRAVERSAL_DATASET_NAME,
        _RANDOM_DENSE_DATASET_NAME,
    }
# --------------------------------------------------------------------------------
...
def _render_shortest_path_result_section(
    ...
    if path_result.steps:
        shortest_visual_key = (
            ...
            f"{st.session_state['ga_shortest_visual_run_id']}"
        )
...
def _store_paired_traversal_results(algorithm_key: str, start_vertex: str) -> None:
    ...
    st.session_state["ga_traversal_list_result"] = list_result
    st.session_state["ga_traversal_matrix_result"] = matrix_result
    # BACKWARD COMPATIBILITY: keep the original single-result slots populated.
    st.session_state["ga_traversal_result"] = list_result
    st.session_state["ga_traversal_result_graph"] = list_graph
# --------------------------------------------------------------------------------
...
def _render_traversal_result_section(
    ...
    selected_step_index = _selected_visual_step_index(
        f"{title} Visual Step",
        len(result.steps),
        key=f"ga_traversal_visual_step_{result.algorithm}_{title}_{result.start_vertex}_{len(result.steps)}",
    )
    # Get the selected step   
...
def _save_benchmark_artifacts() -> None:
    ...
    Returns:
        None.
    """
    benchmark_df = st.session_state["ga_benchmark_df"]
    winners_df = st.session_state["ga_operation_winners_df"]
    scaling_df = st.session_state["ga_scaling_df"]
    ...
    st.success(
        ...
        "operation_scaling_summary.csv under graph_algorithms_module/analysis/."
    )
# --------------------------------------------------------------------------------
...
def _load_saved_benchmark_artifacts() -> None:
    ...
    st.session_state["ga_benchmark_df"] = benchmark_df
    st.session_state["ga_operation_winners_df"] = winners_df
    st.session_state["ga_scaling_df"] = scaling_df
    st.session_state["ga_chart_paths"] = chart_paths
    st.session_state["ga_benchmark_validation"] = run_benchmark_validation(benchmark_df)
...
def _record_operation(
    ...
    *,
    ...
    result_key = f"ga_last_{prefix}_{representation}_operation_result"
...
def _record_paired_structure_operation(
    ...
    *,
    ...
    list_result = st.session_state["ga_last_structure_list_operation_result"]
    matrix_result = st.session_state["ga_last_structure_matrix_operation_result"]
    # Record the results in session state
    st.session_state["ga_last_structure_operation_results"] = (list_result, matrix_result)
    st.session_state["ga_last_structure_section"] = section_id
    ...
    st.session_state["ga_structure_sync_warning"] = " ".join(warnings)
...
def _render_structure_section_result(section_id: str) -> None:
    ...
    if st.session_state.get("ga_last_structure_section") != section_id:
        return
    # Get the results from session state
    result_pair = st.session_state.get("ga_last_structure_operation_results")
...
def _render_structure_lab() -> None:
    ...
    with controls_col:
        ...
        with st.container(border=True):
            ...
            with query_cols[0]:
                has_edge_source = st.text_input(
                    "Has Edge Source",
                    value="Denver",
                    key="ga_structure_has_edge_source",
                )
                has_edge_target = st.text_input(
                    "Has Edge Target",
                    value="Vail",
                    key="ga_structure_has_edge_target",
                )
            ...
            with query_cols[1]:
                weight_query_source = st.text_input(
                    "Weight Query Source",
                    value="Denver",
                    key="ga_structure_weight_query_source",
                )
                weight_query_target = st.text_input(
                    "Weight Query Target",
                    value="Vail",
                    key="ga_structure_weight_query_target",
                )
        ...
        with st.container(border=True):
            ...
            neighbor_vertex = st.text_input(
                "Neighbor Vertex",
                value="Denver",
                key="ga_structure_neighbor_vertex",
            )
    ...
    with view_col:
        ...
        if st.session_state["ga_structure_sync_warning"]:
            st.warning(st.session_state["ga_structure_sync_warning"])
        ...
        render_operation_history(
            ...
            st.session_state["ga_structure_history"],
...
def render_graph_algorithms_page() -> None:
    """Render the Graph Algorithms page inside the root portfolio app.
    ...
    with tab_overview:
        ...
        st.markdown(
            """
    This page integrates the CTA-7 graph representation and algorithm tool into
    the final Portfolio Module 8 application. It preserves the adjacency-list
    and adjacency-matrix labs, traversal and shortest-path demos, benchmark
    workflow, written analysis, and recommendation guide while using Module 8
    package-qualified imports and namespaced Streamlit state.
            """
        )
    ...
    with tab_builder:
        ...
        with col_b:
            vertex_count = st.number_input(
                ...
                min_value=2,
                max_value=500,
                value=int(st.session_state["ga_builder_vertex_count"]),
                step=1,
            )
            seed = st.number_input(
                "Random Seed",
                min_value=0,
                value=int(st.session_state["ga_builder_seed"]),
                step=1,
            )
        # Render the generate graph button
        if st.button("Generate Graph", type="primary"):
            st.session_state["ga_builder_vertex_count"] = int(vertex_count)
            st.session_state["ga_builder_seed"] = int(seed)
        ...
        if _graph_data_ready():
            ...
            st.code(preview_edges(st.session_state["ga_edges"], count=12))
    ...
    with tab_traversal:
        ...
        start_vertex = (
            st.selectbox("Start Vertex", vertices, key="ga_traversal_start_paired")
            if vertices
            else ""
        )
        ...
        with col_demo:
            if st.button("Run Guided Traversal Demo", key="ga_run_guided_traversal_demo_btn"):
                demo = run_traversal_demo("list", directed=False)
                matrix_demo = run_traversal_demo("matrix", directed=False)
                st.session_state["ga_traversal_demo_result"] = demo
                st.session_state["ga_traversal_list_result"] = demo.bfs_result
                st.session_state["ga_traversal_matrix_result"] = matrix_demo.bfs_result
                st.session_state["ga_traversal_result"] = demo.bfs_result
                st.session_state["ga_traversal_result_graph"] = list_graph
        ...
        render_paired_adjacency_views(list_graph, matrix_graph)
        # Get the demo result
        demo_result = st.session_state["ga_traversal_demo_result"]
        ...
        with result_cols[0]:
            _render_traversal_result_section(
                ...
                st.session_state["ga_traversal_list_result"],
                list_graph,
            )
        ...
        with result_cols[1]:
            _render_traversal_result_section(
                ...
                st.session_state["ga_traversal_matrix_result"],
                matrix_graph,
            )
    ...
    with tab_shortest:
        ...
        with path_rep_col:
            path_graph_choice = st.radio("Representation", ["Adjacency List", "Adjacency Matrix"], horizontal=True, key="ga_path_representation")
        # Get the representation
        path_representation = "list" if path_graph_choice == "Adjacency List" else "matrix"
        # Get the demo name
        if st.session_state["ga_shortest_demo_name"] not in _SHORTEST_PATH_DEMOS:
            st.session_state["ga_shortest_demo_name"] = _POSITIVE_DISTANCE_DEMO_NAME
        # Get the demo
        with path_demo_col:
            path_demo_name = st.selectbox(
                "Guided Shortest Path Demo",
                _SHORTEST_PATH_DEMOS,
                key="ga_shortest_demo_name",
            )
        # Get the algorithm
        ...
        # Get the run button
        if st.button("Run Guided Shortest Path Demo", disabled=path_graph is None, key="ga_run_guided_shortest_path_demo_btn"):
            ...
            # Otherwise, run the demo
                ...
                st.session_state["ga_shortest_comparison_df"] = run_positive_shortest_path_comparison(path_representation)
            st.session_state["ga_shortest_demo_result"] = demo
            st.session_state["ga_shortest_path_result"] = demo.result
            st.session_state["ga_shortest_path_result_graph"] = path_graph
            st.session_state["ga_shortest_visual_run_id"] += 1
            st.success("Guided shortest path demo complete.")
        
        # Get the demo result
        shortest_demo = st.session_state["ga_shortest_demo_result"]
        # Render the validation results
        ...
        shortest_comparison_df = st.session_state["ga_shortest_comparison_df"]
        ...
        primary_path_result = st.session_state["ga_shortest_path_result"]
        ...
        display_graph = st.session_state["ga_shortest_path_result_graph"] or path_graph
    ...
    with tab_benchmark:
        ...
        if preset_col_a.button("Preview [25, 50]", key="ga_preview_benchmark_btn"):
            st.session_state["ga_benchmark_sizes"] = [25, 50]
            st.session_state["ga_benchmark_kinds"] = ["sparse"]
            st.info("Preview preset selected: sparse graphs at V=25 and V=50.")
        if preset_col_b.button("Report [25, 50, 100]", key="ga_report_benchmark_btn"):
            st.session_state["ga_benchmark_sizes"] = [25, 50, 100]
            st.session_state["ga_benchmark_kinds"] = list(DEFAULT_GRAPH_KINDS)
            st.info("Report preset selected: sparse and dense graphs at V=25, V=50, and V=100.")
        if preset_col_c.button("Extended [25, 50, 100, 250, 500]", key="ga_extended_benchmark_btn"):
            st.session_state["ga_benchmark_sizes"] = list(DEFAULT_SIZES)
            st.session_state["ga_benchmark_kinds"] = list(DEFAULT_GRAPH_KINDS)
        ...
        with config_col:
            ...
            sizes = st.multiselect(
                "Benchmark Sizes",
                DEFAULT_SIZES,
                default=st.session_state["ga_benchmark_sizes"],
            )
            # Get the graph kinds
            kinds = st.multiselect(
                "Graph Kinds",
                DEFAULT_GRAPH_KINDS,
                default=st.session_state["ga_benchmark_kinds"],
            )
            ...
            repeats = st.number_input(
                ...
                min_value=1,
                max_value=5,
                value=int(st.session_state["ga_benchmark_repeats"]),
                step=1,
            )
            # Get the directed benchmark
            directed_bench = st.checkbox(
                "Directed Benchmark",
                value=bool(st.session_state["ga_benchmark_directed"]),
            )
            # Get the include bellman ford
            include_bellman_ford = st.checkbox(
                "Include Bellman-Ford shortest-path workload",
                value=bool(st.session_state["ga_benchmark_include_bellman_ford"]),
            )
        ...
        if run_clicked:
            ...
            st.session_state["ga_benchmark_sizes"] = selected_sizes
            st.session_state["ga_benchmark_kinds"] = selected_kinds
            st.session_state["ga_benchmark_repeats"] = int(repeats)
            st.session_state["ga_benchmark_directed"] = bool(directed_bench)
            st.session_state["ga_benchmark_include_bellman_ford"] = bool(include_bellman_ford)
        ...
        # Get the validation
        validation = st.session_state["ga_benchmark_validation"]
        ...
        render_benchmark_charts(st.session_state["ga_chart_paths"])
...
# --------------------------------------------------------------- end render_graph_algorithms_page()
