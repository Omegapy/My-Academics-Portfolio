...
# Date: 2026-04-21
# File Path: Portfolio-Milestone-Module-6/streamlit_app.py
# -------------------------------------------------------------------------
...
#
# --- Usage / Integration ---
# - Run with: ``streamlit run Portfolio-Milestone-Module-6/streamlit_app.py``
# - This module executes top-to-bottom under Streamlit rather than through
#   a ``main()`` or ``__main__`` guard.
# - It coordinates the live UI while reading and refreshing saved analysis
#   artifacts from the Module 6 analysis directory.
# -------------------------------------------------------------------------
#
# --- Apache-2.0 ---
...
"""Streamlit application entry module for Portfolio Milestone Module 6.
...
# IMPORTS
# ========================================================================
# Streamlit resolves local module imports only after the module root is added
# to ``sys.path``, so bootstrap that path before importing Module 6 packages.
#
...
# SETUP: ensure Module 6 imports resolve whether Streamlit launches from the
# repo root or from inside the module directory.
if str(_MODULE_ROOT) not in sys.path:
    sys.path.insert(0, str(_MODULE_ROOT))
    
# import analysis modules
from analysis.benchmark_search import (  # noqa: E402
    DEFAULT_QUERY_MODES,
    DEFAULT_REPEATS,
    DEFAULT_SIZES,
    load_results_csv,
...
# import analysis modules
from analysis.lab_validation import (  # noqa: E402
...
# import analysis modules
from analysis.report_generator import (  # noqa: E402
...
from data_structures import BinarySearchTree, ListMap, Map  # noqa: E402
from models.lab_operation_result import LabOperationResult  # noqa: E402
...
#
st.set_page_config(
    page_title="Portfolio Milestone Module 6",
...
def _init_session_state() -> None:
    ...
    defaults: dict[str, object] = {
        ...
        "dataset_keys": [],
        "dataset_items": [],
        "dataset_type": "integers",
        "insertion_pattern": "random",
...
def _build_tree_from_dataset() -> BinarySearchTree:
    ...
    for key in st.session_state["dataset_keys"]:
        tree.insert(key)
    return tree
...
- Task checklist — `{(_MODULE_ROOT / 'task.md').name}`
- Written analysis — `{_WRITTEN_ANALYSIS.name}`
- Recommendation guide — `{_RECOMMENDATION_GUIDE.name}`
- Reference PDFs — `ref_docs/` (trees, BSTs, AVL trees, and the Module 6 lecture)
        """
    )
...
with tabs[1]:
    ...
    insertion_pattern = st.selectbox(
        "Insertion Pattern",
        INSERTION_PATTERNS,
        index=INSERTION_PATTERNS.index(st.session_state["insertion_pattern"]),
    )
    ...
    dataset_seed = st.number_input(
        "Dataset Seed",
        value=int(st.session_state["dataset_seed"]),
        step=1,
    )
    ...
    if st.button("Generate Dataset", key="generate_dataset_btn"):
        ...
        if is_valid:
            st.session_state["dataset_keys"] = keys
            st.session_state["dataset_items"] = _items_from_keys(keys)
            st.session_state["dataset_type"] = dataset_type
            st.session_state["insertion_pattern"] = insertion_pattern
            st.session_state["dataset_size"] = int(dataset_size)
            st.session_state["dataset_seed"] = int(dataset_seed)
            st.session_state["bst"] = None
            st.session_state["tree_map"] = None
            st.session_state["list_map"] = None
            st.success(message)
        else:
            st.error(message)

    # manual dataset
    if st.button("Load Manual Dataset", key="manual_dataset_btn"):
        try:
            ...
            if not is_valid:
                ...
                st.session_state["dataset_items"] = _items_from_keys(keys)
                st.session_state["dataset_type"] = dataset_type
                st.session_state["insertion_pattern"] = "manual"
                st.session_state["dataset_size"] = len(keys)
    ...
    if st.session_state["dataset_keys"]:
        render_dataset_info(
            ...
            st.session_state["insertion_pattern"],
        )
    else:
...
with tabs[2]:
    ...
    if st.button("Build BST from Dataset", key="build_bst_btn"):
        ...
        if not st.session_state["dataset_keys"]:
            ...
            st.session_state["bst"] = after_tree
            st.session_state["last_bst_result"] = _capture_tree_result(
                operation="build_from_dataset",
                input_key=None,
                input_value=None,
                returned_value=len(after_tree),
                success=len(after_tree) == len(st.session_state["dataset_keys"]),
    ...
    # run guided bst demo
    if st.button("Run Guided BST Demo (50 items)", key="guided_bst_demo_btn"):
        st.session_state["guided_bst_results"] = run_bst_demo()
        st.success("Guided BST demo complete.")
    
    # run guided traversal demo
    if st.button("Run Guided Traversal Demo", key="guided_traversal_demo_btn"):
        st.session_state["guided_traversal_results"] = run_traversal_demo()
        st.success("Guided traversal demo complete.")
    
    # run guided unbalanced demo
    if st.button("Run Guided Unbalanced Demo", key="guided_unbalanced_demo_btn"):
        st.session_state["guided_balance_results"] = run_unbalanced_tree_demo()
        st.warning("Unbalanced-tree demo complete.")

    # manual operations
    manual_key_raw = st.text_input("BST Manual Key", key="bst_manual_key_input")
    col1, col2, col3 = st.columns(3)

    # BST insert
    if col1.button("BST Insert", key="bst_insert_btn"):
        tree = st.session_state["bst"]
        # VALIDATION: manual operations require a live BST instance first.
        if tree is None:
            st.warning("Build the BST before running manual operations.")
        else:
            try:
                key = _parse_single_key(manual_key_raw, st.session_state["dataset_type"])
    ...
    # BST search
    if col2.button("BST Search", key="bst_search_btn"):
        tree = st.session_state["bst"]
        ...
        if tree is None:
            ...
            try:
                ...
                st.error(str(exc))
    # BST delete
    if col3.button("BST Delete", key="bst_delete_btn"):
        tree = st.session_state["bst"]
    ...
    render_manual_operation_result(st.session_state["last_bst_result"])
    
    # render tree state panel
    bst: BinarySearchTree | None = st.session_state["bst"]
    if bst is not None:
        render_tree_state_panel(
            name="BST",
            size=len(bst),
    ...
    render_guided_operation_results(
        ...
        st.session_state["guided_bst_results"],
    )
    render_guided_operation_results(
        "Traversal Guided Demo Results",
        st.session_state["guided_traversal_results"],
    )
    render_guided_operation_results(
        "Balance Guided Demo Results",
        st.session_state["guided_balance_results"],
    )
...
with tabs[3]:
    ...
    if st.button("Build TreeMap from Dataset", key="build_tree_map_btn"):
        # VALIDATION: map structures depend on dataset items, not just raw keys.
        if not st.session_state["dataset_items"]:
            st.warning("Generate a dataset before building the TreeMap.")
        else:
            tree_map = Map(st.session_state["dataset_items"])
            list_map = ListMap(st.session_state["dataset_items"])
            st.session_state["tree_map"] = tree_map
            st.session_state["list_map"] = list_map
            st.success("Built TreeMap and ListMap from the dataset.")
    # run guided map demo
    if st.button("Run Guided Map Demo", key="guided_map_demo_btn"):
        st.session_state["guided_map_results"] = run_map_demo()
        st.success("Guided Map demo complete.")
    # manual map operations
    map_key_raw = st.text_input("Map Manual Key", key="map_manual_key_input")
    map_value_raw = st.text_input("Map Manual Value", key="map_manual_value_input")
    col1, col2, col3 = st.columns(3)

    if col1.button("Map Put", key="map_put_btn"):
        tree_map = st.session_state["tree_map"]
        list_map = st.session_state["list_map"]
        ...
        if tree_map is None or list_map is None:
            ...
            try:
                ...
                key = _parse_single_key(map_key_raw, st.session_state["dataset_type"])
                ...
                st.session_state["last_map_result"] = _capture_map_result(
                    operation="put",
                    input_key=key,
                    input_value=value,
                ...
                st.error(str(exc))
    # map get
    if col2.button("Map Get", key="map_get_btn"):
        tree_map = st.session_state["tree_map"]
        ...
        if tree_map is None:
            ...
            try:
                ...
                st.error(str(exc))
    # map delete
    if col3.button("Map Delete", key="map_delete_btn"):
        tree_map = st.session_state["tree_map"]
        list_map = st.session_state["list_map"]
        ...
        if tree_map is None or list_map is None:
            ...
            try:
                ...
                st.error(str(exc))
    # render manual operation result    
    render_manual_operation_result(st.session_state["last_map_result"])
    # render tree map state panel
    tree_map: Map | None = st.session_state["tree_map"]
    list_map: ListMap | None = st.session_state["list_map"]
    # render tree map state panel
    ...
    render_guided_operation_results(
        ...
        st.session_state["guided_map_results"],
    )
...
with tabs[4]:
    ...
    # run benchmark suite
    if st.button("Run Benchmark Suite", key="run_benchmark_btn"):
        ...
        st.session_state["benchmark_df"] = results_df
        st.session_state["benchmark_validation"] = run_benchmark_validation(results_df)
        st.success("Benchmark suite complete.")
    # save benchmark csv
    col1, col2 = st.columns(2)
    if col1.button("Save Benchmark CSV", key="save_benchmark_csv_btn"):
    ...
    # load benchmark csv    
    if col2.button("Load Benchmark CSV", key="load_benchmark_csv_btn"):
        ...
        if not _BENCHMARK_CSV.exists():
            ...
            st.session_state["benchmark_validation"] = run_benchmark_validation(loaded_df)
    ...
    if st.session_state["benchmark_validation"] is not None:
        st.markdown("**Benchmark Validation Summary**")
        st.info(summarize_benchmark_validation(st.session_state["benchmark_validation"]))
    else:
