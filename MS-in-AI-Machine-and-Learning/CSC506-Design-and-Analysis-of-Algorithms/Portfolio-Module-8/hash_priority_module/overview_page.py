...
# Import-safe page module for the Hash Priority area in Portfolio Module 8.
...
"""Import-safe Hash Priority page module for Portfolio Module 8.

This module adapts the CTA-5 Hash Table and Priority Queue Streamlit app so
it can render inside the single Portfolio Module 8 root application.
"""
...
from hash_priority_module.analysis.lab_validation import (
...
# Data
from hash_priority_module.data.dataset_manager import (
    generate_key_value_dataset,
    generate_priority_items,
    generate_collision_keys,
)
# Models
from hash_priority_module.models.lab_operation_result import LabOperationResult
from hash_priority_module.models.priority_item import PriorityItem
from hash_priority_module.analysis.benchmark_search import (
    DEFAULT_RANDOM_SEED,
    DEFAULT_REPEATS,
    run_benchmarks,
    save_results_csv,
...
# Analysis
from hash_priority_module.analysis.report_generator import (
...
# UI
from hash_priority_module.ui.streamlit_helpers import (
    render_header,
    render_dataset_info,
...
# location so the root Streamlit app can launch from any current working directory.
# Rationale: keeps artifact loading reproducible across machines and submission
# environments without environment variables.
# ------------------------------------------------------------------------------
...
_DEFAULTS: dict[str, object] = {
    "hp_kv_dataset": None,
    "hp_priority_items": None,
    "hp_dataset_size": 100,
    "hp_dataset_seed": 506,
    "hp_hash_table": None,
    "hp_priority_queue": None,
    "hp_pq_mode": "max",
    "hp_benchmark_df": None,
    "hp_speedup_df": None,
    "hp_operation_scaling_df": None,
    "hp_hash_demo_result": None,
    "hp_hash_demo_note": "",
    "hp_priority_demo_result": None,
    "hp_priority_demo_note": "",
    "hp_benchmark_validation": None,
    "hp_collision_demo_enabled": False,
    "hp_collision_demo_capacity": 11,
    "hp_collision_demo_target_bucket": 0,
    "hp_hash_history": [],
    "hp_priority_history": [],
    "hp_last_hash_feedback": None,
    "hp_last_priority_feedback": None,
    "hp_last_hash_operation_result": None,
    "hp_last_priority_operation_result": None,
    "hp_benchmark_size_selection": list(DEFAULT_SIZES),
    "hp_benchmark_query_mode_selection": list(DEFAULT_QUERY_MODES),
    "hp_benchmark_repeats": DEFAULT_REPEATS,
}

# --------------------------------------------------------------- _init_session_state()
def _init_session_state() -> None:
    """Initialize Hash Priority session state defaults.
...
def _clear_hash_lab_state(*, clear_table: bool = False) -> None:
    ...
    if clear_table:
        st.session_state["hp_hash_table"] = None
    st.session_state["hp_hash_demo_result"] = None
    st.session_state["hp_hash_demo_note"] = ""
    st.session_state["hp_hash_history"] = []
    st.session_state["hp_last_hash_feedback"] = None
    st.session_state["hp_last_hash_operation_result"] = None
# --------------------------------------------------------------------------------
...
def _clear_priority_lab_state(*, clear_queue: bool = False) -> None:
    ...
    # SAFETY CHECK: only forget the structure when the caller explicitly opts in
    if clear_queue:
        st.session_state["hp_priority_queue"] = None
    st.session_state["hp_priority_demo_result"] = None
    st.session_state["hp_priority_demo_note"] = ""
    st.session_state["hp_priority_history"] = []
    st.session_state["hp_last_priority_feedback"] = None
    st.session_state["hp_last_priority_operation_result"] = None
# --------------------------------------------------------------------------------
...
def _hash_dataset_ready() -> bool:
    ...
    return st.session_state["hp_kv_dataset"] is not None
# --------------------------------------------------------------------------------
...
def _priority_dataset_ready() -> bool:
    ...
    return st.session_state["hp_priority_items"] is not None
# --------------------------------------------------------------------------------
...
def _hash_table_ready() -> bool:
    ...
    return st.session_state["hp_hash_table"] is not None
# --------------------------------------------------------------------------------
...
def _priority_queue_ready() -> bool:
    ...
    return st.session_state["hp_priority_queue"] is not None
# --------------------------------------------------------------------------------
...
def _select_hash_demo_records() -> tuple[list[tuple[str, int]] | None, str]:
    ...
    """
    records = st.session_state["hp_kv_dataset"]
...
def _select_priority_demo_items() -> tuple[list[PriorityItem] | None, str]:
    ...
    Logic:
        ...
        3. Otherwise return ``None`` so the caller generates a 100-item set.
    """
    items = st.session_state["hp_priority_items"]
...
def _store_benchmark_outputs(benchmark_df: pd.DataFrame) -> None:
    ...
    st.session_state["hp_benchmark_df"] = benchmark_df
    st.session_state["hp_speedup_df"] = speedup_df
    st.session_state["hp_operation_scaling_df"] = operation_scaling_df
    st.session_state["hp_benchmark_validation"] = validation_result
...
def _set_benchmark_preview_sizes() -> None:
    ...
    st.session_state["hp_benchmark_size_selection"] = [100, 500]
# --------------------------------------------------------------------------------
...
def _get_example_hash_key() -> str | None:
    ...
    hash_table: HashTable | None = st.session_state["hp_hash_table"]
...
def _record_hash_operation_result(
    ...
    *,
    ...
    """
    active_table: HashTable = st.session_state["hp_hash_table"]
    ...
    st.session_state["hp_last_hash_operation_result"] = LabOperationResult(
...
def _record_priority_operation_result(
    ...
    *,
    ...
    Logic:
        ...
        4. Build the LabOperationResult and stash it in session state.
    """
    active_queue: BinaryHeapPriorityQueue = st.session_state["hp_priority_queue"]
    ...
    summary_text = summary(returned_value) if callable(summary) else summary
    # package the captured fields into a LabOperationResult
    st.session_state["hp_last_priority_operation_result"] = LabOperationResult(
...
def render_hash_priority_page() -> None:
    """Render the Hash Priority page inside the root portfolio app.
    ...
    with tab_overview:
        ...
        st.markdown(
            """
    This Hash Priority page integrates the CTA-5 hash table and priority queue
    workflows into the final Portfolio Module 8 web app. It compares these
    data structures:
    ...
    **Binary Heap Priority Queue**
    ...
    with tab_dataset:
        ...
        with col_size:
            dataset_size = st.slider(
                ...
                min_value=10,
                max_value=5_000,
                value=st.session_state["hp_dataset_size"],
                step=10,
                key="hp_dataset_size_slider",
            )
        ...
        with col_seed:
            dataset_seed = st.number_input(
                "Random Seed",
                min_value=0,
                value=st.session_state["hp_dataset_seed"],
                step=1,
                key="hp_dataset_seed_input",
            )
        # collision demo checkbox
        use_collision_demo = st.checkbox(
            "Use Forced-Collision Hash Demo",
            value=st.session_state["hp_collision_demo_enabled"],
            key="hp_collision_demo_enabled_checkbox",
            help=(
        ...
        collision_capacity = int(st.session_state["hp_collision_demo_capacity"])
        collision_target_bucket = int(st.session_state["hp_collision_demo_target_bucket"])
        ...
        if use_collision_demo:
            ...
            with col_capacity:
                collision_capacity = int(st.number_input(
                    "Collision Demo Capacity",
                    min_value=3,
                    value=int(st.session_state["hp_collision_demo_capacity"]),
                    step=1,
                    key="hp_collision_demo_capacity_input",
                ))
            ...
            with col_bucket:
                collision_target_bucket = int(st.number_input(
                    ...
                    value=min(
                        int(st.session_state["hp_collision_demo_target_bucket"]),
                        max(collision_capacity - 1, 0),
                    ),
                    step=1,
                    key="hp_collision_demo_target_bucket_input",
                ))
        # generate dataset button
        if st.button("Generate Dataset", type="primary", key="hp_generate_dataset_btn"):
            st.session_state["hp_dataset_size"] = dataset_size
            st.session_state["hp_dataset_seed"] = dataset_seed
            st.session_state["hp_collision_demo_enabled"] = use_collision_demo
            st.session_state["hp_collision_demo_capacity"] = collision_capacity
            st.session_state["hp_collision_demo_target_bucket"] = collision_target_bucket
            ...
            if use_collision_demo:
                ...
                st.session_state["hp_kv_dataset"] = [
                    (collision_key, value)
                    for collision_key, (_, value) in zip(collision_keys, base_records)
                ]
            else:
                st.session_state["hp_kv_dataset"] = base_records

            st.session_state["hp_priority_items"] = generate_priority_items(
        ...
        render_dataset_info(
            st.session_state["hp_kv_dataset"],
            st.session_state["hp_priority_items"],
        )
        # collision demo info
        if st.session_state["hp_collision_demo_enabled"] and st.session_state["hp_kv_dataset"] is not None:
            st.info(
                "Forced-collision demo is active. In the Hash Table Lab, use "
                f"`initial capacity = {st.session_state['hp_collision_demo_capacity']}` "
    ...
    with tab_hash:
        ...
        # hash table state
        ht: HashTable | None = st.session_state["hp_hash_table"]
        hash_demo_result = st.session_state["hp_hash_demo_result"]
        ...
        render_lab_status_summary(
            [
                ...
                (
                    "Dataset Size",
                    f"{len(st.session_state['hp_kv_dataset']):,}" if _hash_dataset_ready() else "0",
                ),
                ("Hash Table", "Loaded" if _hash_table_ready() else "Not Loaded"),
                (
                    "Collision Demo",
                    "Active" if st.session_state["hp_collision_demo_enabled"] else "Standard",
                ),
            ]
        )
        # setup section
        with st.container(border=True):
            ...
            with cfg1:
                initial_capacity = st.number_input(
                    ...
                    value=53,
                    step=1,
                    key="hp_ht_initial_capacity_input",
                )
            # enable auto-resize
            with cfg2:
                enable_resize = st.checkbox(
                    "Enable Auto-Resize",
                    value=True,
                    key="hp_ht_enable_resize_checkbox",
                )
            ...
            # collision demo info
            if st.session_state["hp_collision_demo_enabled"]:
                st.info(
            ...
            with load_col1:
                if st.button(
                    ...
                    if records is not None:
                        ...
                        _clear_hash_lab_state()
                        st.session_state["hp_hash_table"] = hash_table
                        _set_feedback(
                            "hp_last_hash_feedback",
                            "success",
            ...
            with load_col2:
                ...
                if st.button(
                    "Load Forced-Collision Demo",
                    key="hp_ht_collision_demo_load_btn",
                    disabled=not (
                        _hash_dataset_ready() and st.session_state["hp_collision_demo_enabled"]
                    ),
                    help=(
                        "Enable Forced-Collision Hash Demo in Dataset Builder first."
                        if not st.session_state["hp_collision_demo_enabled"]
                    ...
                    # load forced-collision demo
                    if records is not None:
                        demo_capacity = int(st.session_state["hp_collision_demo_capacity"])
                        ...
                        st.rerun()
        # hash table state
        ht = st.session_state["hp_hash_table"]
        hash_demo_result = st.session_state["hp_hash_demo_result"]
        # guided demo section
        with st.container(border=True):
            render_section_intro(
            ...
            with demo_col1:
                ...
                if st.button(
                    ...
                    demo_table, demo_result = run_hash_table_demo(
                        ...
                        dataset_size=100,
                        seed=int(st.session_state["hp_dataset_seed"]),
                        initial_capacity=int(initial_capacity),
                        max_load_factor=max_lf,
                    )
                    # clear hash lab state
                    _clear_hash_lab_state()
                    st.session_state["hp_hash_table"] = demo_table
                    st.session_state["hp_hash_demo_result"] = demo_result
                    st.session_state["hp_hash_demo_note"] = demo_note
                    _set_feedback(
                        "hp_last_hash_feedback",
                        "success",
            ...
            with demo_col2:
                ...
                if st.button(
                    "Run Guided Collision Demo",
                    key="hp_ht_run_collision_demo_btn",
                ):
                    # run guided collision demo
                    demo_capacity = (
                        int(st.session_state["hp_collision_demo_capacity"])
                        if st.session_state["hp_collision_demo_enabled"]
                        else max(int(initial_capacity), 11)
                    )
                    # run guided collision demo
                    demo_bucket = int(st.session_state["hp_collision_demo_target_bucket"])
                    demo_table, demo_result = run_hash_table_demo(
                        dataset_size=12,
                        seed=int(st.session_state["hp_dataset_seed"]),
            ...
            if hash_demo_result is not None:
                # hash demo note
                if st.session_state["hp_hash_demo_note"]:
                    st.caption(st.session_state["hp_hash_demo_note"])
        ...
        with st.container(border=True):
            ...
            with example_col1:
                ...
                if st.button(
                    ...
                    # use example existing key
                    st.session_state["hp_ht_search_key"] = example_key or ""
                    st.session_state["hp_ht_del_key"] = example_key or ""
        
            with example_col2:
                # use example missing key
                if st.button(
                    "Use Example Missing Key",
                    key="hp_ht_use_missing_btn",
                    ...
                    # use example missing key
                    st.session_state["hp_ht_search_key"] = "missing-hash-key"
                    st.session_state["hp_ht_del_key"] = "missing-delete-key"
            # operation columns
                ...
                if insert_submitted:
                    ...
                    # insert value
                        ...
                        _record_hash_operation_result(
                            ...
                            # match key
                            match_key=ins_key,
                            op_callable=lambda: st.session_state["hp_hash_table"].insert(
                                ins_key,
                                insert_value,
                            ),
                        )
            ...
            with op_col2:
                ...
                with st.form("ht_search_form"):
                    ...
                    search_key = st.text_input(
                        "Key to Search",
                        key="hp_ht_search_key",
                        disabled=not manual_hash_ready,
                    )
                    # search submitted
                    search_submitted = st.form_submit_button(
                        "Search",
                        key="hp_ht_search_btn",
                        disabled=not manual_hash_ready,
                    )
                # search submitted
                if search_submitted:
                    result = _record_hash_operation_result(
                        ...
                        op_callable=lambda: st.session_state["hp_hash_table"].search(search_key),
                    )
                    # result is not None
            ...
            with op_col3:
                ...
                if delete_submitted:
                    deleted_value = _record_hash_operation_result(
                        ...
                        match_key=del_key,
                        op_callable=lambda: st.session_state["hp_hash_table"].delete(del_key),
                    )
            ...
            # render feedback
            _render_feedback("hp_last_hash_feedback")
            hash_operation_result = st.session_state["hp_last_hash_operation_result"]
            ...
            render_operation_history(
                ...
                list(st.session_state["hp_hash_history"]),
        ...
        with st.container(border=True):
            ...
            if ht is not None:
                render_hash_table_state_panel(
                    ht.get_stats(),
                    bool(st.session_state["hp_collision_demo_enabled"]),
                )
                # render action tip
                render_action_tip(
    ...
    **Separate Chaining** — each bucket stores a linked list (Python list) of
    ...
    with tab_pq:
        ...
        pq: BinaryHeapPriorityQueue | None = st.session_state["hp_priority_queue"]
        pq_mode = st.session_state["hp_pq_mode"]
        ...
        render_lab_status_summary(
            [
                ...
                (
                    "Dataset Size",
                    f"{len(st.session_state['hp_priority_items']):,}" if _priority_dataset_ready() else "0",
                ),
        ...
        with st.container(border=True):
            ...
            selected_mode = st.radio(
                "Heap Mode",
                options=["max", "min"],
                index=0 if st.session_state["hp_pq_mode"] == "max" else 1,
                horizontal=True,
                key="hp_pq_mode_radio",
            )
            # set heap mode
            st.session_state["hp_pq_mode"] = selected_mode
            ...
            if st.button(
                ...
                if items is not None:
                    ...
                    for item in items:
                        queue.insert(item)
                    _clear_priority_lab_state()
                    st.session_state["hp_priority_queue"] = queue
                    # set feedback
                    _set_feedback(
                        "hp_last_priority_feedback",
                        "success",
        ...
        # get priority queue
        pq = st.session_state["hp_priority_queue"]
        # render guided demo
        with st.container(border=True):
            # render section intro
            render_section_intro(
            ...
            if st.button(
                ...
                _clear_priority_lab_state()
                # set state
                st.session_state["hp_priority_queue"] = demo_queue
                st.session_state["hp_priority_demo_result"] = demo_result
                st.session_state["hp_priority_demo_note"] = demo_note
                # set feedback
                _set_feedback(
                    "hp_last_priority_feedback",
                    "success",
            ...
            priority_demo_result = st.session_state["hp_priority_demo_result"]
            # if priority demo result is not None
            if priority_demo_result is not None:
                # if priority demo note is not None
                if st.session_state["hp_priority_demo_note"]:
                    # render caption
                    st.caption(st.session_state["hp_priority_demo_note"])
                # render validation results
                render_validation_results(
        ...
        with st.container(border=True):
            ...
            with example_col1:
                ...
                if st.button(
                    ...
                    st.session_state["hp_pq_search_del"] = example_label or ""
            # with example column 2
            with example_col2:
                # render button
                if st.button(
                    "Use Missing Label",
                    key="hp_pq_use_missing_btn",
                    ...
                    st.session_state["hp_pq_search_del"] = "no-such-task"
            # with example column 3
            with example_col3:
                # render button
                if st.button(
                    "Insert High-Priority Example",
                    key="hp_pq_use_insert_example_btn",
                    disabled=not manual_priority_ready,
                    help=(
                        "Load or generate a queue first."
                        if not manual_priority_ready
                    ...
                    # set state
                    st.session_state["hp_pq_label"] = label
                    st.session_state["hp_pq_priority"] = priority
                    st.session_state["hp_pq_payload"] = payload
            ...
            with op2:
                ...
                if peek_submitted:
                    # get active queue
                    active_queue = st.session_state["hp_priority_queue"]
                    # if queue is empty
            ...
            # render feedback
            _render_feedback("hp_last_priority_feedback")
            # get priority operation result
            priority_operation_result = st.session_state["hp_last_priority_operation_result"]
            ...
            render_operation_history(
                ...
                list(st.session_state["hp_priority_history"]),
    ...
    """)
        ...
        with ctrl1:
            ...
            bench_sizes = st.multiselect(
                ...
                key="hp_benchmark_size_selection",
            )
        ...
        with ctrl2:
            bench_modes = st.multiselect(
                ...
                key="hp_benchmark_query_mode_selection",
            )
        ...
        with ctrl3:
            bench_repeats = st.number_input(
                ...
                step=1,
                key="hp_benchmark_repeats",
            )

        # render run benchmark button
        run_col1, run_col2 = st.columns(2)
        with run_col1:
            if st.button("Run Benchmark", type="primary", key="hp_run_benchmark_btn"):
        ...
        with run_col2:
            if st.button("Run Assignment Benchmark Suite", key="hp_run_assignment_suite_btn"):
                # render progress
        ...
        # --- Load Previous ---
        if _CSV_PATH.exists() and st.session_state["hp_benchmark_df"] is None:
            if st.button("Load Previous Results", key="hp_load_previous_results_btn"):
                df = load_results_csv(_CSV_PATH)
                _store_benchmark_outputs(df)

        # --- Display Results ---
        bench_df = st.session_state["hp_benchmark_df"]
        # if benchmark dataframe
        if bench_df is not None and not bench_df.empty:
            # get benchmark validation
            benchmark_validation = st.session_state["hp_benchmark_validation"]
            ...
            speedup_df = st.session_state["hp_speedup_df"]
            ...
            operation_scaling_df = st.session_state["hp_operation_scaling_df"]
